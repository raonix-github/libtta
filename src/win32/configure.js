/* Configure script for tta.dll, specific for Windows with Scripting Host.
 *
...
/* Input and output files for the tta_version.h include */
var newfile;
...
function usage() {
	...
	txt += "\nTTA library configure options, default value given in parentheses:\n\n";
...
function readVersion() {
	...
	while (cf.AtEndOfStream !== true) {
		ln = cf.ReadLine();
		s = new String(ln);
		if (s.search(/^m4_define\(\[libtta_version_major/) != -1) {
			verMajor = s.substr(s.indexOf(",") + 3, 1);
		} else if (s.search(/^m4_define\(\[libtta_version_minor/) != -1) {
			verMinor = s.substr(s.indexOf(",") + 3, 1);
		} else if (s.search(/^m4_define\(\[libtta_version_micro/) != -1) {
...
function createVersionedFile(newfile, unversioned) {
	...
	while (ofi.AtEndOfStream !== true) {
		ln = ofi.ReadLine();
		s = new String(ln);
		if (!dryRun && s.search(/\@LIBTTA_VERSION_MAJOR\@/) != -1) {
			of.WriteLine(s.replace(/\@LIBTTA_VERSION_MAJOR\@/, verMajor));
		} else if (!dryRun && s.search(/\@LIBTTA_VERSION_MINOR\@/) != -1) {
			of.WriteLine(s.replace(/\@LIBTTA_VERSION_MINOR\@/, verMinor));
		} else if (!dryRun && s.search(/\@LIBTTA_VERSION_MICRO\@/) != -1) {
			of.WriteLine(s.replace(/\@LIBTTA_VERSION_MICRO\@/, verMicro));
		} else if (!dryRun && s.search(/\@LIBTTA_VERSION\@/) != -1) {
			of.WriteLine(s.replace(/\@LIBTTA_VERSION\@/, verMajor + "." + verMinor + "." + verMicro));
		} else {
			if (!dryRun) {
				of.WriteLine(ln);
			}
...
newfile = srcDir + "\\tta_version.h";
createVersionedFile(newfile, srcDir + "\\tta_version.h.in");
if (error !== 0) {
	WScript.Echo("Creation of " + newfile + " failed, aborting.");
	WScript.Quit(error);
}

newfile = "tta.dll.manifest";
createVersionedFile(newfile, "tta.dll.manifest.in");
if (error !== 0) {
	WScript.Echo("Creation of " + newfile + " failed, aborting.");
	WScript.Quit(error);
}
...
WScript.Echo("\nLibtta configuration completed\n");
